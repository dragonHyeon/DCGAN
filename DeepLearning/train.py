    ...
    def _train(self):
        ...
        * 학습 진행
        ...
        for x in tqdm(self.train_dataloader, desc='train dataloader', leave=False):
            ...
            # GAN loss
            loss_D_GAN_real = bce(self.modelD(x), real_label)
            fake_x = self.modelG(z)
            loss_D_GAN_fake = bce(self.modelD(fake_x.detach()), fake_label)
            ...
            # GAN loss
            fake_x = self.modelG(z)
            loss_G_GAN = bce(self.modelD(fake_x), real_label)
    ...
    def _eval(self, sample_z_collection):
        ...
        * 테스트 진행
        ...
        for x in tqdm(self.test_dataloader, desc='test dataloader', leave=False):
            ...
            # GAN loss
            score_D_GAN_real = bce(self.modelD(x), real_label)
            fake_x = self.modelG(z)
            score_D_GAN_fake = bce(self.modelD(fake_x), fake_label)
            score_D_GAN = score_D_GAN_real + score_D_GAN_fake
            ...
            score_G_GAN = bce(self.modelD(fake_x), real_label)
